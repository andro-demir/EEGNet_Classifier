...
class AdversarialCNN:
    def __init__(self, chans, samples, n_output, n_nuisance, architecture='EEGNet', adversarial=False, lam=0.01):
    ...
    def encoder_model(self, architecture):
        ...
        if architecture == 'EEGNet':
            model.add(Conv2D(8, (1, 75), padding='same', use_bias=False))
    ...
    def train(self, train_set, val_set, log_file, epochs=500, batch_size=40):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for iter, (batch_start, batch_end) in enumerate(train_batches):
                ...
                if epoch == 1 and iter == 1:   # logging for epoch zero (i.e., the first iteration)
                    self.logging(log_file, epoch=0, train_log=np.mean(train_log, axis=0),
                                 val_log=self.acnn.test_on_batch(x_test, [y_test, s_test]))
            train_log = np.mean(train_log, axis=0)
            val_log = self.acnn.test_on_batch(x_test, [y_test, s_test])
            self.logging(log_file, epoch=epoch, train_log=train_log, val_log=val_log)
    ...
    def logging(self, log_file, epoch, train_log, val_log):
